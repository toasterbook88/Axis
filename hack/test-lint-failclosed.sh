...
# 1) crashing gofmt must fail — prepend fakebin, keep rest of PATH for make
printf '%s\n' '#!/bin/sh' 'echo crash >&2' 'exit 127' > "$tmp/fakebin/gofmt"
chmod +x "$tmp/fakebin/gofmt"
if PATH="$tmp/fakebin:$PATH" make -C "$tmp/src" lint >"$tmp/lint-out" 2>&1; then
...
# 2) unformatted file must fail (use real gofmt from PATH)
