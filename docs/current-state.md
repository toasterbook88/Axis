...
- Publication evidence on live and daemon-cached snapshots: unique publication ID, assembly source/time, projected cache age, facts observation time/discovery digest, ledger digest/count, and state digest/schema/update time
...
| `axis update` | Self-update binary | Default: bounded in-place update of the **running** install only (SHA-256 via `checksums.txt`). Reports other known installs as shadows. `--all` refreshes validated older shadows (PATH + `~/.local/bin`/`~/go/bin`/`/usr/local/bin`/`/opt/homebrew/bin`) after per-target AXIS identity, version (no silent downgrade), and package-manager path checks. `--path` for one path; `--check` reports multi-install status |
...
| `internal/publication` | Build snapshot publication evidence | Fingerprints the pre-overlay facts, frozen ledger entry set, and state input without claiming cross-store transactional coherence |
...
- `axis task run`, HTTP `/run`, and approved `axis agent` shell execution (`run_shell` local pin, `run_on_node` requested-node pin, and remote `run_background` via the same RunOnNode runner) now share the same guarded execution path and persist both internal caller provenance and runtime-derived local origin identity; both `axis task run` and `axis agent` prefer the local daemon/API `/run` client when available, and the streamed NDJSON contract preserves ready-time placement output, explicit state changes, and live stdout/stderr across that hop, but broader runtime identity is still limited to one AXIS token trust domain rather than a richer distributed or multi-hop model. Wiring is enforced by contract tests on `buildAgentSessionConfig` (RunShell/RunOnNode/SystemExtra cannot be dropped silently); `/model` refreshes guarded runners so OwnerLabel provenance tracks the live model.
...
- `axis status`, `axis task place`, and `axis task context` now overlay local reservation state on live reads and can surface typed discovery freshness plus publication component evidence in machine-readable output. Metadata and snapshot responses are still separate reads and are not yet bound to one publication ID.
