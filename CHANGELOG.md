...
* **Gossip peer-exchange no longer multiplies nodes per address:** merged peers are identified by `stable_id` when present, otherwise by cluster-unique node name — never by hostname, which each sender's `nodes.yaml` authors as the address that author can reach. Self-gossip arriving back under an alternate local address (tailscale, secondary NIC) is ignored, and entries learned from gossip no longer inherit the sender's `config` provenance, so the failure detector can evict them once they stop being refreshed. A table that previously accumulated one entry per address variant plus self-echo now converges to one entry per physical node.
* **Beacon listener shares the discovery port with the CLI scanner:** the daemon's beacon listener binds with `SO_REUSEADDR`/`SO_REUSEPORT`, so `axis mesh peers --live` can scan while the daemon holds the beacon port instead of failing with address-already-in-use. A live scan briefly takes a share of beacons; the beacon interval and daemon refresh window tolerate it.
